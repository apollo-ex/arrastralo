...
const submissionSchema = z.object({
  role: z.enum(['importer', 'carrier']),
  email: z.string().email(),
  phone: z.string().regex(/^\d{10,15}$/, 'phone must be only digits (10-15)'),
  city: z.string().regex(/^[A-Za-zÁÉÍÓÚáéíóúÑñ ]{2,}$/, 'city must contain only letters/spaces'),
  ...
  paymentTerms: z.enum(['7_days', '15_days', '30_days', 'immediate']).optional(),
